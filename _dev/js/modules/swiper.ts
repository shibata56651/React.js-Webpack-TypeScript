...
export class swiper {
	o: {
		timer: boolean; playBtnClass: string; playTxtClass: string; isPauseClass: string; isActiveSlideClass: string; playTxt: string; // 再生
		pauseTxt: string; // 一時停止
		activeSlideLength: { // アクティブにしたいスライドの数をswiperオプションのブレイクポイントごとに必ず指定する
			breakpoints: { max: number; };
		}; swiperOptions: {};
	};
	root: any;
	modalHook: NodeListOf<Element>;
	swiper: Swiper;
	autoPlay: any;
	playBtn: any;
	playTxt: any;
	isPlay: boolean;
	totalActiveLength: number;
	addActiveLength: number;
